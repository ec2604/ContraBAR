...
from utils.helpers import rgb2gray
...
class CustomReach(Task):
    ...
    def reset_task(self, sample_goal=True) -> None:
        if sample_goal:
            self.goal = self._sample_goal()
        self.sim.set_base_pose("target", self.goal, np.array([0.0, 0.0, 0.0, 1.0]))
...
class CustomReachEnv(RobotTaskEnv):
    ...
    def _get_obs(self):
        ...
        # ee_velocity = np.array(self.robot.get_ee_velocity())
        # return ee_position
    ...
    def visualise_behaviour(env,
        ...
        if kwargs.get('save_video', False):
            ...
            belief_video = np.concatenate([np.stack(imgs[0][:-1]), np.stack(imgs[1][:-1]), np.stack(imgs[2][:-1])])
            size = belief_video.shape[1:3]
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
            print("saving to: ", image_folder + '/test_seed_80_orig.avi', flush=True)
            out = cv2.VideoWriter(image_folder + '/test_seed_80_orig.avi', fourcc, fps, (size[1], size[0]))
...
class CustomReachWindEnv(CustomReachEnv):
    def __init__(self, render: bool = False, reward_type: str = "sparse", control_type: str = "ee",
                 table_color=None, plane_color=None, wind_strength=0.1, sample_goal=False) -> None:
        super().__init__(render, reward_type, control_type, table_color, plane_color)
        r = 0.15
        theta = 0
        x = np.cos(theta) * r
        y = np.sin(theta) * r
    ...
    def step(self, action: np.ndarray):
        return super().step(action + self.wind)
    ...
    def reset_task(self, seed=None):
        ...
        with self.sim.no_rendering():
            self.robot.reset()
            self.task.reset_task(sample_goal=self.sample_goal)
        return self._get_obs()
    ...
    def get_task(self):
        ...
        Return a task description, such as goal position or target velocity.
        """
        return np.concatenate([self.task.goal, self.wind])
    ...
    def visualise_behaviour(env,
        ...
        #
        # curr_task = torch.tensor(env.get_task(),device=device)
        # predictor_input, _, (x,y,z) = utl.generate_predictor_wind_input(episode_hidden_states, curr_task[:,3:])
        # rewards = torch.sigmoid(kwargs['belief_evaluator'](predictor_input)).squeeze(1)
        # thetas = np.linspace(-np.pi / 2, np.pi / 2, 1000)
        # r = 0.15
        # sc_x = r*np.cos(thetas)
        ...
        #     plt.imshow(rewards[i].reshape(len(x),len(x)).detach().cpu().numpy(), extent=extent, cmap='gray', vmin=0, vmax=1, origin='lower')
        #     plt.colorbar()
        #     ax.add_patch(plt.Circle(xy=curr_task[0][0:3].detach().cpu().numpy(), radius=0.005, alpha=0.4, color='g'))
            # plt.scatter(curr_task[0][3].detach().cpu().numpy(), curr_task[0][4].detach().cpu().numpy(), color='r')
            # x_pos, y_pos = pos_episodes[(i//49)*49:i+1, 0 ], pos_episodes[49*(i // 49) :i+1, 1]
            # plt.plot(sc_x, sc_y, alpha=0.5, c='m',)
            # plt.plot(x_pos, y_pos, alpha=0.8,label=f'Episode {i // 50}, step: {i}')
            # plt.legend()
        ...
        if kwargs.get('save_video', False):
            ...
            size = belief_video.shape[1:3]
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
            print("saving to: ", image_folder + '/test_seed_80.avi', flush=True)
            out = cv2.VideoWriter(image_folder + '/test_seed_80.avi', fourcc, fps, (size[1], size[0]))
            for img in belief_video:
                img = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)
                out.write(img)
            out.release()
