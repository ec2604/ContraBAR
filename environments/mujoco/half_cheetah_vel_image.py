...
class HalfCheetahVelEnvImage(HalfCheetahEnv):
    ...
    def __init__(self, max_episode_steps=200):
        ...
        super(HalfCheetahVelEnvImage, self).__init__()
    ...
    def visualise_behaviour(self,env,
        ...
        for episode_idx in range(num_episodes):
            ...
            for step_idx in range(1, env._max_episode_steps + 1):
                ...
                if info[0]['done_mdp'] and not done:
                    ...
                    start_state = torch.from_numpy(start_state).float().to(device).unsqueeze(0)
                    start_pos = unwrapped_env.get_body_com("torso")[0].copy()
                    break
        ...
        kwargs['logger'].add_video('behaviour_video_rollout_1',
                                   np.expand_dims(
                                       np.transpose(np.stack(episode_prev_img[0], axis=0), axes=[0, -1, 1, 2]), 0),
                                   iter_idx)
        kwargs['logger'].add_video('behaviour_video_rollout_2',
                                   np.expand_dims(
                                       np.transpose(np.stack(episode_prev_img[1], axis=0), axes=[0, -1, 1, 2]), 0),
                                   iter_idx)
...
class HalfCheetahRandVelOracleEnv(HalfCheetahVelEnvImage):
